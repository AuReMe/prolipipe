...
def scatter_with_status(df_res, pwy_name, filename = "", status_colname="Status", species_colname="Species", ):
    ## check number of status and attribute color to each
    all_status = list(set(df_res[status_colname]))
    nb_status = len(all_status)
    if nb_status > 10 :
        print(f"ERROR : please note that the scatterplot representation can only hold up to 10 different status ; as here {nb_status} are found in the '{status_colname}' column of taxon file, please consider reducung this number to 10 or below.\nAborting")
        exit(1)
    else :
        colors = ["blue", "orange", "green", "red", "purple", "brown", "pink", "gray", "olive", "cyan"]
        status2colors = {all_status[i] : colors[i] for i in range (0, len(all_status))}
    traces = []
...
def bar_presence_reactions(res_df, pwy_name, max_comp="", barplot_presence_path = ""): 
    ...
    fig = px.bar(dict(Reactions=rxn2ratio.keys(), Presence_pct=rxn2ratio.values()), x="Reactions", y="Presence_pct", 
    color="Presence_pct", color_continuous_scale=px.colors.sequential.Greens, range_color=(0,100),
...
## initalize global summary df and build output directory
