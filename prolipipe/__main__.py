...
options:
  ...
  --is_padmet
                        Boolean to call if the input directory contains merged padmet files from AuFAMe
...
def parser() : 
    ...
    ## arguments 
    parser.add_argument("-d", "--dir", required=True, dest="directory",help="Path to working directory for Prolipipe containing tsv_files directory or the merged padmet directory (need --is_padmet option) generated by AuFAMe.")
    parser.add_argument("--is_padmet", dest="is_padmet", action="store_true", help="Use it if inputs are padmet files")
...
def compare_padmet(input_dir):
    """
    Compare padmet files from each annotation tool and create tsv files
    """

    padmet_command = f"""
    mkdir -p {input_dir}/tsv_files
    padmet compare_padmet --padmet={input_dir}/merged_padmet --output={input_dir}/tsv_files -v
    """
    try:
...
def main():
    ...
    if options.is_padmet:
        print("\nRun compare padmets")
        compare_padmet(options.directory)
